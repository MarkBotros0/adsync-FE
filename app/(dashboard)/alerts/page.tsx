...
const TYPE_ICONS: Record<Alert['type'], any> = {
  mention_spike: Zap,
...
const TYPE_COLORS: Record<Alert['type'], string> = {
  mention_spike:  'bg-blue-100 text-blue-600',
  negative_surge: 'bg-red-100 text-red-600',
  keyword:        'bg-slate-100 text-slate-600',
  influencer:     'bg-green-100 text-green-600',
};
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center gap-4">
        <h1 className="text-base font-bold text-slate-900">Alerts</h1>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Active Alerts', value: alerts.filter(a => a.enabled).length.toString(), color: 'text-green-600' },
            { label: 'Triggered Today', value: '2', color: 'text-blue-600' },
            { label: 'Inactive Alerts', value: alerts.filter(a => !a.enabled).length.toString(), color: 'text-slate-500' },
            { label: 'Total Alerts', value: alerts.length.toString(), color: 'text-slate-700' },
          ].map(stat => (
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100">
            <h2 className="text-sm font-semibold text-slate-800">Your Alerts</h2>
          </div>
          <div className="divide-y divide-slate-100">
            {alerts.map(alert => {
              ...
              return (
                <div key={alert.id} className={`flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors ${!alert.enabled ? 'opacity-60' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">{alert.condition} · {alert.platform}</p>
                  </div>
                  <div className="text-right mr-4">
                    <p className="text-xs text-slate-400">Last triggered</p>
                    <p className="text-xs font-medium text-slate-600">{alert.triggered}</p>
                  </div>
                  <button
                    onClick={() => toggleAlert(alert.id)}
                    className={`shrink-0 transition-colors ${alert.enabled ? 'text-purple-600 hover:text-purple-800' : 'text-slate-300 hover:text-slate-500'}`}
                  >
                    {alert.enabled
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-blue-800">Real-time Notifications</p>
