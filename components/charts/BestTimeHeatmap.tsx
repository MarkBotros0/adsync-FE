  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        <div>
          <h3 className="text-sm font-semibold text-slate-800">Best Time To Post</h3>
      ...
      <div className="p-5 overflow-x-auto">
        ...
        {DAYS.map((day, dayIdx) => (
          <div key={day} className="flex items-center mb-1">
            <span className="text-[11px] text-slate-500 w-10 shrink-0">{day}</span>
        ...
        <div className="flex items-center gap-1.5 mt-3 ml-10">
          ...
          {[0, 5, 12, 22, 35, 50].map(v => (
            <div key={v} className="h-3 w-4 rounded-sm" style={{ background: getColor(v) }} />
          ))}
